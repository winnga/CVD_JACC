...
querydaly<- get_outputs(topic='cause', release_id=9, compare_version_id=compare_version, 
                        cause_id = c(492,  493,  495,  496,  497,  498,  968,  969,  970,
                                     942,  938,  944, 1004,  500,  501,  502,503,  507),
                        location_id='all', year_id=2022, sex_id=3, age_group_id=27, measure_id=2, metric_id=3)
...
dalyw_EU <- subset(querydaly, location_type.x == "region")
...
query2comp <- subset(query2021, location_type.x == "region"| location_type.x == "global")
# multiple by 100000 for rates
...
#########################################################
##########TABLE DATA
query2020daly <- get_outputs(topic='cause', release_id=9, compare_version_id=compare_version, 
...
globalsdi2$region_name<- ifelse(globalsdi2$location_name.x=="Global", "Global", globalsdi2$region_name)

sdiscatter <- ggplot(globalsdi2[location_type.x=="admin0"|location_type.x=="global"], aes(x= SDI, y=mapvar2, col=super_region_name)) +
  geom_smooth(aes(group = 1), colour = "blue4",method = "loess", formula = y ~ x, se = TRUE, alpha=.2)+
  geom_point(size = 3, alpha=.50)+
  geom_point(data = globalsdi2[location_name.x == "Global"], aes(x=SDI, y=mapvar2, col= region_name), shape = 17, size = 2)+
  scale_colour_manual(breaks = c("Central Europe, Eastern Europe, and Central Asia", "High-income", "Latin America and Caribbean", 
                                 "North Africa and Middle East ", "South Asia ", "Southeast Asia, East Asia, and Oceania", "Sub-Saharan Africa", "Global"),
                     ...
                     labels = c("Central Europe, Eastern Europe, and Central Asia", "High-income", "Latin America and Caribbean",
                                "North Africa and Middle East ", "South Asia ", "Southeast Asia, East Asia, and Oceania", "Sub-Saharan Africa", "Global"))+
  labs(shape="Super Region", colour="GBD Super-Regions and Global")+
...
ggsave(paste0(outdir, locid, "/", "Figure4.png"), plot=riskplot, height=5, width=8, dpi=500)
...
p1 <- query3[year_id == 2022] %>%
  ...
  labs(y="Age-Standardized Mortality Rate per 100,000", x="Cardiovascular Diseases", color = "Regions")+
...
breaks = seq(min(query2020pyr$valrate),max(query2020pyr$valrate), length.out = n)
