...
# Purpose: Produce pdf files with plots and maps for JACC Cause Appendices. This can be run for each risk by specifying 
#          the output directory and cause id.
##########################################################################################################################
##########################################################################################################################
rm(list=ls())
...
suppressMessages(sourceDirectory("/ihme/cc_resources/libraries/current/r/", modifiedOnly=FALSE))
...
#data for plot 1
mapdata<-get_outputs(topic='cause', release_id=9, compare_version_id=compare_version,
                    cause_id = cause_id,  year_id = 2022, age_group_id = 27, sex_id = 3, 
                    metric_id = 3, measure_id = 1, location_id='all')

#data for plot 2
agesexdiff<-get_outputs(topic='cause', release_id=9, compare_version_id=compare_version,
                        cause_id = cause_id,  year_id = 2022, sex_id=c(1,2), age_group_id=c(1, 188, 9:20, 30:32, 235),
                        metric_id = 3, measure_id = c(1,5,2), location_id=1)
...
source("ADDRESS")
cause_meta <- get_cause_metadata(cause_set_id = 3, release_id = 9)
plot_cause <- copy(cause_id)
fatal_age_start <- cause_meta[cause_id == plot_cause, yll_age_start]
nonfatal_age_start <- cause_meta[cause_id == plot_cause, yld_age_start]
min_age_start <- min(fatal_age_start, nonfatal_age_start, na.rm=T)
...
agesexdiff[measure_id == 5 & age_group_years_start < nonfatal_age_start & age_group_years_end <= nonfatal_age_start, `:=` (val = 0, sex = 'Not estimated')]
...
#data for plot 3
regionbysex<-get_outputs(topic='cause', release_id=9, compare_version_id=compare_version,
                    cause_id = cause_id,  year_id = 2022, sex_id=c(1,2), age_group_id=27,
                    measure_id=2, metric_id=3, location_id=c(5,9,21,32,42,56,65,70,73,96,100,104,120,124,134,138,159,167,174,192,199))

#data for plot 4

panels <- get_outputs(topic='cause', release_id=9, compare_version_id=compare_version, 
                     cause_id=cause_id, location_id=1, year_id=c(1990:2022), sex_id=3, 
                     age_group_id=c(22, 27), measure_id=c(1:5), metric_id=3)

causename <- unique(mapdata$cause_name)

causename <- str_to_sentence(causename)

causename <- ifelse(causename=="Cardiovascular diseases", "Cardiovascular disease", causename)

causenametitle <- ifelse(causename== "Cardiovascular disease" | causename== "Stroke"|causename=="Cardiomyopathy and myocarditis"|causename=="Non-rheumatic valvular heart disease", paste0('Aggregate Cause: ', causename), causename)
#########################################################
########## PLOT 1: MAP DATA #############################
...
mapdata<- mapdata[!grepl("RUS_", mapdata$ihme_loc_id),]
...
message(paste0(lim2, sep=" | "))
message(paste0(labs, sep=" | "))

pdf(file = paste0(outdir, cause_id, "_map1.pdf"), width=8,height=6)
gbd_map(mapdata,
        ...
        title= str_to_upper(causenametitle),
        pattern=NULL, 
        #col="RdYlBu", 
        ...
        #fname=paste0(outdir, cause_id, "_map1.tif"))
dev.off()

###########################################################################################################
########## PLOT 2: AGE/SEX DIFFERENCES 

agesexdiff<- agesexdiff[!is.na(agesexdiff$val), ]
...
p <- ggplot(agesexdiff2)+
  ...
  labs(caption = str_wrap(paste0("Figure 4. Global ", tolower(causename), " age-specific estimates per 100,000 by sex in 2022,  difference in sex estimates indicated by gray bar"),100))+
  scale_x_continuous(label= ~ ifelse(.x < 3, scales::comma(.x), scales::comma(.x, accuracy = 1)), breaks = scales::pretty_breaks(n = 8)) + 
  ...
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        ...
        axis.text.x=element_text(angle=45,  hjust = 1),plot.margin=unit(c(.5,.5,.5,.5), "cm"),
        strip.text = element_text(size = 8),text=element_text(size=12),plot.caption = element_text(size = 14, hjust = 0))

tiff(paste0(outdir, cause_id,"_plot1.tif"), width=10,
     height=8,
     units="in",
     res=300)
p
...
#######################################################
##########graph by sex 
regionbysex$valrate <- regionbysex$val*100000
...
plot2 <- ggplot(regionbysex, aes( y = valrate, x = reorder(location_name, sort_order), fill=sex)) +
  ...
  labs(fill = "Sex",  caption = str_wrap(paste0("Figure 3. ", causename," age-standardized disability-adjusted life years (DALYs) per 100,000 by Global Burden of Disease region and sex in 2022, 95% uncertainty interval indicated by black bar"),100))+
  theme_bw(base_size=12)+
  theme(axis.text.x=element_text(angle=45,  hjust = 1))+
  theme(panel.grid.major = element_blank(), panel.grid.minor = element_blank(),
        panel.background = element_blank(), axis.line = element_line(colour = "black"), 
        plot.title.position = "plot",legend.title=element_blank(),
        plot.margin=unit(c(.5,.5,.5,.5), "cm"),text=element_text(size=12),plot.caption = element_text(size = 14, hjust = 0))
plot2

tiff(paste0(outdir, cause_id, "_plot2.tif"), width=10,
     height=8,
     units="in",
     res=300)
plot2
...
shift_legend <- function(p){
  ...
  panels$fixed_measure_name = factor(panels$fixed_measure_name, levels=c( "YLDs (Years Lived with Disability)", "Prevalence", "YLLs (Years of Life Lost)","Mortality",  "DALYs (Disability-Adjusted Life Years)"))
  ...
  q2 <- ggplot(data=panels1, aes(x=year_id, y=val, colour=age_group_name)) + 
    ...
    scale_color_brewer(palette="Set1") + scale_y_continuous(label= ~ ifelse(.x < 10, scales::comma(.x), scales::comma(.x, accuracy = 1)),limits=c(ifelse(min(panels1$lower)<0, NA, 0), NA)) + 
    geom_hline(yintercept = 0 , size = 0.2, alpha = 0.5)+
  ...
  q2new <- q2 +
    guides(fill = guide_legend(title.position = "top",
                               ...
                               nrow = 1)) +
    labs(caption= str_wrap(paste0("Figure 2. Global ", tolower(causename)," estimates per 100,000 by measure with shaded 95% uncertainty interval, 1990-2022"),100))+
    theme(legend.direction = "horizontal",plot.caption = element_text(hjust = 0, size=14), plot.margin=unit(c(.5,.5,.5,.5), "cm"))

  tiff(paste0(outdir, cause_id,"_plot3.tif"), width=10,
       height=8,
       units="in",
       res=300)
...
p_1 <- grid::rasterGrob(image_read_pdf(paste0(outdir,cause_id,'_map1.pdf')))
p_2 <- grid::rasterGrob(readTIFF(paste0(outdir,cause_id,'_plot1.tif')))
p_3 <- grid::rasterGrob(readTIFF(paste0(outdir,cause_id,'_plot2.tif')))
p_4 <- grid::rasterGrob(readTIFF(paste0(outdir,cause_id,'_plot3.tif')))    
...
pdf(paste0(outdir,cause_id,'_twopagerappendix.pdf'), width = 8.5, height=11)
tryCatch(grid.arrange( top=title,p_1,p_4,ncol=1, bottom=title)+ grid.text(str_wrap(paste0("Figure 1. Global map of age-standardized ", tolower(causename)," mortality rate per 100,000 in 2022 with equal interval classification"),100),x=unit(.2, 'npc'), y=unit(.56, 'npc'),gp = gpar(col = "black", fontsize =8), just="left"), error=function(e){})
